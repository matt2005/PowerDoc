...
function Test-PrivateIPAddress ([System.Net.IPAddress]$IPAddress) 
{
  ...
  Write-Output -InputObject $PrivateIPAddress 
}
...
function Get-ComputerDomain ([string]$ComputerName) 
{
  ...
  Write-Output -InputObject $ComputerDomain
...
function Get-ActiveDirectoryDnsConfiguration 
{
  ...
  process {
    ...
    Write-Output -InputObject $ActiveDirectoryDnsConfiguration
  }
}
...
function Get-IPv4SubnetConfiguration 
{
  ...
  process {
    ...
    Write-Output -InputObject $IPv4SubnetConfiguration
  }
}
...
function Find-IPv4Device 
{
  ...
  process {
    ...
    if (-not $MaxConcurrencyThrottle) 
    {
      $MaxConcurrencyThrottle = 1 
    }
    ...
    if ($ExcludeSubnet) 
    {
      Write-NetworkScanLog -Message "`t-ExcludeSubnet: $([String]::Join(',',$ExcludeSubnet))" -MessageLevel Information 
    }
    if ($LimitSubnet) 
    {
      Write-NetworkScanLog -Message "`t-LimitSubnet: $([String]::Join(',',$LimitSubnet))" -MessageLevel Information 
    }
    if ($ExcludeComputerName) 
    {
      Write-NetworkScanLog -Message "`t-ExcludeComputerName: $([String]::Join(',',$ExcludeComputerName))" -MessageLevel Information 
    }
    ...
    if ($LimitSubnet) 
    {
      $LimitSubnet | ForEach-Object -Process {
        $LimitSubnetNetwork += (Get-NetworkSummary -Network $_)
      }
    }
    if ($ExcludeSubnet) 
    {
      $ExcludeSubnet | ForEach-Object -Process {
    ...
    if ($ExcludeComputerName) 
    {
      ...
      try 
      {
        ...
        ForEach-Object -Process {
          ...
          Where-Object {
            $_.AddressFamily -ieq 'InterNetwork' 
          } |
          ForEach-Object {
    ...
    switch ($PsCmdlet.ParameterSetName) {
      'dns' 
      {
        ...
        $ActiveDirDnsConfig | ForEach-Object -Process {
          ...
          ForEach-Object {
            ...
            if ($HasMetCriteria -eq $true) 
            {
              if (-not ($Device.Values |
                  Where-Object {
                    ($_.DnsRecordName -ieq $HostName) -and ($_.IPAddress -ieq $IPAddress) 
              })) 
              {
      ...
      { 
        ...
        $IPv4SubnetConfig | ForEach-Object -Process {
    ...
    # Queue up PING tests
    $Device.GetEnumerator() | ForEach-Object -Process {
      $ScanCount++
    ...
    $PingAliveDevice = @($Device.GetEnumerator() | Where-Object -FilterScript {
        $($_.Value).IsPingAlive -eq $true 
    })
    ...
    $PingAliveDevice | ForEach-Object -Process {
      $ScanCount++
    ...
    $ScriptBlock = {
      ...
      $Win32_ComputerSystem = Get-CimInstance -Namespace root\CIMV2 -ClassName Win32_ComputerSystem -Property Name -ComputerName $DNSHostName -OperationTimeoutSec 10 -ErrorAction Stop
      $ComputerName = $Win32_ComputerSystem.Name
      Write-Output -InputObject $ComputerName
    }
    ...
    $PingAliveDevice | ForEach-Object -Process {
      ...
      } elseif ($CurrentDevice.WmiMachineName) 
      {
      ...
      } elseif ($CurrentDevice.WinRMMachineName) 
      {
      ...
      # If we do have a DNS Hostname that doesn't begin with the WMI Machine Name and $ResolveAliases is true, get the machine name from DNS
      if (
        (-not $CurrentDevice.DnsRecordName) -or (-not ($CurrentDevice.DnsRecordName -like '*.*'))
      ) 
      {
        try 
        {
          $ResolvedNames= Resolve-DnsName -Name $(Resolve-DnsName -Name $CurrentDevice.IPAddress -NoHostsFile -Erroraction Stop)[0].NameHost -NoHostsFile -Erroraction Stop
          Foreach ($row in $ResolvedNames)
          {
            $CurrentDevice.DnsRecordName = $row.Name.ToUpper()
          }
    ...
    # Count how many devices responded
    $WinRMAliveDevice = @($Device.GetEnumerator() | Where-Object -FilterScript {
        $CurrentDevice.IsPingAlive -eq $true 
    })
    ...
    # Return results
    Write-Output -InputObject $Device.Values
    ...
    Write-NetworkScanLog -Message "`t-PING Replies: $($($Device.Values | Where-Object {
      $_.IsPingAlive -eq $true 
    } | Measure-Object).Count)" -MessageLevel Information
    Write-NetworkScanLog -Message "`t-WMI Replies: $($($Device.Values | Where-Object {
      $_.IsWmiAlive -eq $true 
    } | Measure-Object).Count)" -MessageLevel Information
    Write-NetworkScanLog -Message "`t-WinRM Replies: $($($Device.Values | Where-Object {
      $_.IsWinRMAlive -eq $true 
    } | Measure-Object).Count)" -MessageLevel Information
  ...
  process {
    ...
    switch ($PsCmdlet.ParameterSetName) {
      'dns' 
      {
        ...
        if ($ExcludeSubnet) 
        {
          $ParameterHash.Add('ExcludeSubnet',$ExcludeSubnet) 
        }
        if ($LimitSubnet) 
        {
          $ParameterHash.Add('IncludeSubnet',$LimitSubnet) 
        }
        if ($ExcludeComputerName) 
        {
          $ParameterHash.Add('ExcludeComputerName',$ExcludeComputerName) 
        }
      }
      'subnet' 
      {
      ...
      try 
      {
        $ManagedComputer.Services | ForEach-Object {
          ...
          if ($ServiceTypeName -ieq 'SQL Server') 
          {
            ...
            $ServiceProtocol = $ManagedComputer.ServerInstances |
            Where-Object {
              $_.Name -ieq $ManagedComputerServerInstanceName 
            } |
            ForEach-Object {
              $_.ServerProtocols |
              Where-Object {
    ...
    $Service.Service |
    Select-Object -Property ServiceTypeName -Unique |
    Sort-Object -Property ServiceTypeName |
    ForEach-Object {
      $ServiceTypeName = $_.ServiceTypeName
      Write-NetworkScanLog -Message "`t-$($ServiceTypeName) Instance Count: $(($Service.Service | Where-Object {
        ($_.ServiceTypeName -ieq $ServiceTypeName) 
      } | Measure-Object).Count)" -MessageLevel Information
    }
